...
using static EloBuddy.DamageType;
using static EloBuddy.SDK.Enumerations.HitChance;
using static EloBuddy.SDK.Enumerations.SkillShotType;
using static EloBuddy.SDK.TargetSelector;
...
namespace xRP_Ashe
{
    internal class Program
    {
        ...
        private static readonly float Rangeauto = Me.GetAutoAttackRange();
        ...
        {
            var intTarget = GetTarget(R.Range, Magical);
            {
        ...
        private static void Itens()
        {
            ...
            {
                var targetEr = GetTarget(botrk.Range, Physical);
            ...
            {
                var targetY = GetTarget(Rangeauto, Physical);
                if (targetY.IsValidTarget(Rangeauto))
                {
                    yommus.Cast();
                }
        ...
        private static void Combo()
        {
            ...
            if (Q.IsReady() && useQ)
            {
                var targetq = GetTarget(Rangeauto, Physical);

                if (targetq.IsValidTarget(Rangeauto))
                {
            ...
            if (R.IsReady() && useR)
            {
                var targetr = GetTarget(R.Range, Magical);
        ...
        private static void LaneClear()
        {
            ...
            if (W.IsReady() && farmW)
            {
                ...
                if (minionW.CountEnemyMinionsInRange(W.Range) >= countM)
                {
                    W.Cast(minionW);
                }
            }
        ...
        private static void Harass()
        {
            ...
            if (Q.IsReady() && useQ)
            {
                var targetq = GetTarget(range: Me.GetAutoAttackRange(), damageType: Physical);
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            if (!Me.IsDead)
            {
                ...
                if (drawAa)
                {
                    Drawing.DrawCircle(Me.Position, Rangeauto, Color.Red);
                }
